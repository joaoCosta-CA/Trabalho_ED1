...
// A struct interna é definida no .c, escondendo a implementação.
typedef struct {
    int id;
    PILHA pilha_formas;
} CarregadorStruct;
...
int carregador_esta_vazio(Carregador carregador) {
    ...
    CarregadorStruct* c = (CarregadorStruct*)carregador;
    PILHA pilha_interna = c->pilha_formas; // Obtém o ponteiro da pilha

    return pilha_vazia(pilha_interna); // Passa o ponteiro obtido
}